        ...
        if(countries.containsKey(data[5])){
            ...
            if(data[6].equals("NONE")){
                if(c.getParliament().getParliamentFormation().getId()==Integer.parseInt(data[7])){
                    c.getParliament().getParliamentFormation().getGovernmentAndOpposition().put(ParliamentTypes.GOVERNMENT,new Government(Integer.parseInt(data[0]),data[1],Integer.parseInt(data[2]),Integer.parseInt(data[3]),Integer.parseInt(data[4])));
                }
            }else {
                if (c.getRegions().containsKey(data[6])){
                    ...
                    if (r.getRegionalParliament().getParliamentFormation().getId()==Integer.parseInt(data[7])){
                        r.getRegionalParliament().getParliamentFormation().getGovernmentAndOpposition().put(ParliamentTypes.GOVERNMENT,new Government(Integer.parseInt(data[0]),data[1],Integer.parseInt(data[2]),Integer.parseInt(data[3]),Integer.parseInt(data[4])));
                    }
                }else throw new ReadRegionException("Region " + data[6] + " has not been found for governments!");
            }
        }else {
            throw new ReadCountryException("Country " + data[5]  + " has not been found for Governments!");
        }
        return countries;
    }
}
